	...
	func(onAssigned)
	{
		...
		if (isTypeOf(_task,Saloon_Task_RoofV2) && {!getVar(_task,militiaBriefSent)}) then {
			private _roofMes = format[
				"<t color='#F6D365'>В последние смены группировка 'Рука' и барник из 'Дыры' всё реже платят крышу за собственные проделки. У одних несчётное количество преступлений, на которые мы до этой смены закрывали глаза, у другого алкоголь непонятно откуда, да и ещё и продаёт он его направо и налево всем забулдыгам по звяку за бутылку... Сам, что ли, гонит? Надо бы проверить наши схроны на наличие звяков. Должно быть по %1 звяков. Схрон Пахана: %2 Схрон Барника: %3. Тот, кто положит больше - останется безнаказанным до следующей смены, а вот с должников надо бы спросить. Да ещё и вызов поступил. Проверим схроны и сходим разберёмся?</t>",
